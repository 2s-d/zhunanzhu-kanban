...
// 计算连续签到天数
const calculateConsecutiveCheckInDays = (lastCheckInDate) => {
    if (!lastCheckInDate)
        return 0;
...
// 提取时间趋势数据
export const extractTimeTrendData = (data, timeRange) => {
    ...
    data.projects.forEach(project => {
        project.logs.forEach(log => {
            ...
            // 根据时间范围过滤
            if (!isInTimeRange(log.timestamp, timeRange))
                return;
...
// 检查时间戳是否在指定范围内
const isInTimeRange = (timestamp, timeRange) => {
...
// 分析奖励来源
export const analyzeRewardSources = (data) => {
    if (!data || !data.projects.length)
        return [];
    let checkInPoints = 0;
    let studyPoints = 0;
...
// 分析学习时段分布
export const analyzeStudyPeriodDistribution = (data) => {
    if (!data || !data.projects.length)
        return [];
    const distribution = {
        morning: 0, // 6-12
...
// 计算平均学习会话时长
export const calculateAverageSessionDuration = (data) => {
    if (!data || !data.projects.length)
        return 0;
    let totalMinutes = 0;
    let sessionCount = 0;
